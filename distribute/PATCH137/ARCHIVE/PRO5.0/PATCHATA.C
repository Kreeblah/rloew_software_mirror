...
// blk 1
0x81,0xbb,0xc0,0,0,0,0x4c,0x52,0x4c,0x52,0x75,4,0xc6,0x43,0x20,2,  // Force Verify if MSD = 524C524C
...
0x81,0xbb,0xc0,0,0,0,0x4c,0x52,0x4c,0x52,0x75,0x1B,  // If Offset MSD  = 524C524C
0x51,0x8b,0x4b,0x14,
0x81,0xb9,0xfc,1,0,0,0x4c,0x52,0x4c,0x52,0x75,0xa,   // If  [Sector+1FC] = 524C524C
0xc7,0x81,0xfc,1,0,0,0,5,0x52,0x4c,                  // Set [Sector+1FC] = 4C520500
0x59,
// Version 5.0
...
int off[]={0,0xc,0x40,0x58,0x4b,0x56,0x63};    
#define patchp 0x6a
...
char cr[]="                48-Bit LBA Patch 5.0  Copyright (C) 2003-2006 by Rudolph R. Loew. All Rights Reserved.  €€€€€€€€";
...
if (j-0x85) goto bad;
memcpy(patch+p98a,patch98,10);memcpy(patch+p98b,patch98,10);
*(long *)(patch+p98x)=j;
seme: j= *(short *)(b+0x18);
if (j<0x40) goto bad;
j= *(long *)(b+0x3c);

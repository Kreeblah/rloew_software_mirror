...
unsigned char p1b[]={
0xf6,0x81,0xa7,0x00,0x00,0x00,0x04,0x59,0x75, 0x06, // Test 48-Bit LBA Enable Flag
                                                    // 9 Bytes Replaced for Windows 98
0x25,0xff,0xff,0xff,0xf,0xc3
};
...
0x3c,0x40,0x75,0x4a,                                 // If Verify Sectors
0x81,0xbb,0xc0,0,0,0,0,0,0x4c,0x52,0x75,0x3e,        // If Offset MSD    = 524C0000
0x51,0x8b,0x4b,0x14,
0x81,0xb9,0xfc,1,0,0,0x52,0x4c,0x52,0x4c,0x75,0x17,  // If  [Sector+1FC] = 4C524C52
0x50,0x8a,0x67,0x3d,0x8a,0x86,0xba,0,0,0,            // Set [Sector+1FC] = Slave Port.L
0x66,0x89,0x81,0xfc,1,0,0,0x58,
0x0f,0xba,0x6f,0x2e,0x0a,                            // Set I/O Log Flag
0x81,0xb9,0xfc,1,0,0,0x4c,0x52,0x4c,0x52,0x75,0xa,   // If  [Sector+1FC] = 524C524C

#ifdef DEMO
0xc7,0x81,0xfc,1,0,0,2,5,0x45,0x44,                  // Set [Sector+1FC] = 44450502
#else
0xc7,0x81,0xfc,1,0,0,2,5,0x52,0x4c,                  // Set [Sector+1FC] = 4C520502
#endif
...
unsigned char p7c[]={
0xf6,0x81,0xa7,0x00,0x00,0x00,0x04,0x59,0x75, 0x01,  // Test 48-Bit LBA Enable
                                                     // 9 Bytes Replaced for Windows 98
0xc3
};
...
#ifdef SELF
char cr[]="                48-Bit LBA Patch 5.2  Copyright (C) 2003-2006 by Rudolph R. Loew. All Rights Reserved.          ";
#endif
...
#ifdef DEMO
char cr[]="                48-Bit LBA Patch 5.2  (DEMO)  Copyright (C) 2003-2006 by Rudolph R. Loew. All Rights Reserved.          ";
#endif
...
unsigned char patch98[]={0x81,0x79,0x78,0xff,0xff,0xff,0x0f,0x59,0x74};
#define p98a (p1b-p)
#define p98b (p7c-p)
...
if (j-0x85) goto bad;
memcpy(p+p98a,patch98,9);memcpy(p+p98b,patch98,9);
*(long *)(p+p98x)=j;
seme: j= *(short *)(b+0x18);
if (j<0x40) goto bad;
j= *(long *)(b+0x3c);

...
0xc9,0x74,0xa,0xe8,0xfa,0,0x72,5,0xff,0x17,0x4e,0xeb,0xee,0x61,0xeb,0x12,
...
unsigned long mod48=0xffffffff;
...
char **v;
{
int i,j,k,m,x,cc,xxx[2];
unsigned long e,l,ll,d[8],mult,xx[4];
...
if ((v[0][1]&0xdf)=='E') {mod48=mod48d;goto arg;}
...
if (c<2) {printf("RFDISK [-RAW] [-CHS] [-ExtMBR] [-Overlap] [-NoSwap] [-MaxDrive#] Write|Diag\n");return(20);}

#ifdef DEMO
else printf("\nR. Loew Electronics Drive Partitioning Program Version 2.0 (DEMO)\nCopyright (C) 2008-2010, All Rights Reserved\n\n");
#else
else printf("\nR. Loew Electronics Drive Partitioning Program Version 2.0\nCopyright (C) 2008-2010, All Rights Reserved\n\n");
#endif
...
d[4]=d[5]=d[6]=0;int13(0x48,i,0L,d);
if (d[5]) d[4]=mod48+((d[5]&255)<<16)+((d[4]>>16)&0xff00);
disk->length=d[4];
...
case 'A': cmd[60]=0;
          ...
          if (cc<4) goto bad;
          if (validate(2,mult,l,ll)) goto lp1;
          if ((i==5) || (i==15)) {
          if (l==0) {printf("Invalid Start\n");goto lp1;}
          if (cmd[60]=='+') {printf("Cannot Offset Extended Partition\n");goto lp1;}
          }
          if ((l&0xff000000)-mod48) l*=mult;
          if ((ll&0xff000000)-mod48) ll*=mult;
...
case 'C': ll=0;
          ...
          if ((ll&0xff000000)-mod48) ll=ll*mult;
...
case 'F': l=ll=0;cmd[60]=0;
          ...
          if ((l&0xff000000)-mod48) l*=mult;
          brk=0;signal(2,brkp);
          for (;l<cdisk->length;l+=((l&0xff000000)-mod48) ? mult : 256) {
...
swapd:    for (k=0x1be,x=0;k<0x1fe;k+=16) if (ss[k]) x++;
          ...
          ep=p->lpart;
          while(ep) {
          ext1=(struct psec *)calloc(sizeof(struct psec)-(enab ? 512 : 0),1);
...
if (dd->mb>=2) printf("WARNING: Unrecognized, Obsolete or Corrupted MBR\n");
...
if (dd->sec8) {
if (((dd->cyl8<1023) && (dd->cyl8-dd->cyls)) || (dd->heads-dd->hd8) || (dd->sectors-dd->sec8)) {
...
if (*b) goto alp;
printf("\n");
if ((entry->heads-dd->heads) || (entry->sectors-dd->sectors)) printf("WARNING: Partition Geometry Set To %u Heads %u Sectors\n",entry->heads,entry->sectors);
...
struct part *p,*e;
ss=256;
if ((s&0xff000000)-mod48) {ss=d->heads*d->sectors;s*=ss;}
ss+=s;
p=d->list;
while(p) {
...
return(getnum(s));
fnd: if (mod48==0xffffffff) return(-1);
j=getnum(s+i+1);i=getnum(s);
if (((i|j)& -256)) return(-1);
if (i==0) if ((j<<24)<=mod48) return(-1);
return(mod48+(i<<16)+(j<<8));
}
...
unsigned long i;
i=mod48/m;
if (s==0xffffffff) goto bads;
if (s<mod48) if (s>i) goto bads;
if (l==1) return(0);
if (e==0xffffffff) goto bade;
if (e<mod48) if (e>i) goto bade;
if (s<mod48) if (e>mod48) goto badr;
if (e>s) if (e<=(s+0x10000)) return(0);

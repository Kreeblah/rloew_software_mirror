...
int enab=0,mode=0,fix=0,brk,crf;
unsigned long hide=0xffffffff;
unsigned long mod40=0xffffffff,time0=0;
...
if (c<2) {
printf("RFDISK [-RAW] [-CHS] [-XT] [-ExtMBR] [-Overlap] [-NoSwap] [-FixSig]\n       [-#(MaxDrive)] Write|Diag\n");
return(20);
}

#ifdef DEMO
else printf("\nR. Loew Electronics Drive Partitioning Program Version 2.10 (DEMO)\nCopyright (C) 2008-2013, All Rights Reserved\n\n");
#else
else printf("\nR. Loew Electronics Drive Partitioning Program Version 2.10\nCopyright (C) 2008-2013, All Rights Reserved\n\n");
#endif
...
goto notfnd;
fnd: if (sec.parts[j].ehead-255) disk->heads=sec.parts[j].ehead+1;
...
case 'C': ll=0;
          ...
          if (cc>5) {
          ll=xx[3];
          if (validate(((i==5) || (i==15)) ? 3 : 2,mult,xx[4],ll)) goto lp1;
          if (mode>=2) if (ll>(xt ? 4096 : 1024)) {printf("Invalid CHS Cylinder\n");goto lp1;}
...
printf("Error Reading Boot Blk on Disk %u Sector %lu in Cyl %lu\n",d1->drive-0x80,p1->start,p1->start/p1->heads/p1->sectors);
return(0);
}
i=getsig(&s);
if (i-val) {
...
printf("Error Writing Boot Blk on Disk %u Sector %lu in Cyl %lu\n",d1->drive-0x80,p1->start,p1->start/p1->heads/p1->sectors);
return(0);
}
p1->sig=i;return(i);
}

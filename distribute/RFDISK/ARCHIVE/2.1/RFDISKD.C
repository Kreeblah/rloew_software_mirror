...
#ifdef DEMO
else printf("\nR. Loew Electronics Drive Partitioning Program Version 2.1 (DEMO)\nCopyright (C) 2008-2010, All Rights Reserved\n\n");
#else
else printf("\nR. Loew Electronics Drive Partitioning Program Version 2.1\nCopyright (C) 2008-2010, All Rights Reserved\n\n");
#endif
...
case 'A': cmd[60]=0;
          ...
          if (mode>=2) if (ll>(xt ? 4095 : 1023)) {printf("Invalid CHS Cylinder\n");goto lp1;}
          if ((i==5) || (i==15)) {
...
swapd:    for (k=0x1be,x=0;k<0x1fe;k+=16) if (ss[k]) x++;
          ...
          if (int13(0x42,cdisk->drive,ep->base,ext->s1)) {printf("Error Reading Sector %lu in Cylinder %lu\n",ep->base,ep->base/p->heads/p->sectors);cdisk->err|=1;goto lp3;}
          ...
          *(long *)(ss+12)=l-ep->base;
          ...
          ext1=ext;
          while(ext1) {
          if (int13(0x43,cdisk->drive,ext1->blk,ext1->s1)) {printf("Error Writing Extended Chain Sector %lu in Cylinder %lu\n",ext1->blk,ext1->blk/cdisk->heads/cdisk->sectors);goto lp3;}
          ext1=ext1->next;
          }
          }
          p=cdisk->list;
          ...
          ext1=ext;
          while(ext1) {
          for (i=0;i<512;i++) if (ext1->s1[i]-ext1->s2[i]) printf("Sector %lxh Mismatch %x: %02x %02x\n",ext1->blk,i,ext1->s2[i],ext1->s1[i]);
          ext1=ext1->next;
          }

...
0x72,6,0x56,0xbe,0xbe,0xb,0xeb,0x13,0xe9,0x3e,0xff,0xbf,0xbe,0x7d,0xb9,0x20,
0,0x33,0xc0,0xf3,0xaf,0x74,0xde,0x56,0xbe,0xbe,0x7d,0xbf,0xbe,9,0xeb,7,
0x56,0xbe,0xbe,0xb,0xbf,0xbe,0xd,0xb9,0x20,0,0xf3,0xa5,0x5e,0x80,0x3e,0xfa,
...
#ifdef DEMO
else printf("\nR. Loew Electronics Drive Partitioning Program Version 1.0 (DEMO)\nCopyright (C) 2008, All Rights Reserved\n\n");
#else
else printf("\nR. Loew Electronics Drive Partitioning Program Version 1.0\nCopyright (C) 2008, All Rights Reserved\n\n");
#endif
...
printf("NewTable Reference                 SetActiveTable Reference\n");
...
printf("Find [Start]                       Validate Type Start\n");
...
memcpy(s,mbr2,0x1b4);memcpy(s+512,mbr,0x18a);memcpy(s+0x1b4,s+0x3b4,0x4a);
for (j=0x38a;j<0x3b4;j++) s[j]=0;
...
memcpy(s+512,mbr,0x18a);s[1022]=0x30;
for (j=0x38a;j<0x3fe;j++) s[j]=0;
}
if (int13(0x43,i,0L,1L,s+512)) {printf("Error Writing Default Sector\n");goto ierr;}
imbr: memcpy(s,mbr2,0x1b4);memcpy(s+0x1b4,s+0x3b4,0x4c);s[510]=0x55;s[511]=0xaa;
if (int13(0x43,i,0L,0L,s)) {printf("Error Writing Partition Sector\n");goto ierr;}
}
else {
for (l=1;l<62;l++) {
...
swapd:
          ...
          *(long *)(ss+12)=l-ep->base;
          ...
          l=i;
          if (int13(0x43,cdisk->drive,0L,l,s+(i<<9))) {printf("Error Writing Disk %x Sector %lx\n",cdisk->drive,l);goto lp3;}
          }
          ext1=ext;
          while(ext1) {
          if (int13(0x43,cdisk->drive,0L,ext1->blk,ext1->s1)) {printf("Error Writing Disk %x Sector %lx\n",cdisk->drive,ext1->blk);goto lp3;}
          ext1=ext1->next;
          }
          }
          p=cdisk->list;
          ...
          for (i=0;i<k;i++) {
          if (enab) if (int13(0x43,cdisk->drive,0L,p->start+i,s+(j<<9))) {printf("Error Writing Disk %x Sector %lx\n",cdisk->drive,p->start+i);goto lp3;}
          ...
          for (i=0;i<k;i++) {
          if (enab) if (int13(0x43,cdisk->drive,0L,ep->start+i,s+(j<<9))) {printf("Error Writing Disk %x Sector %lx\n",cdisk->drive,ep->start+i);goto lp3;}
          if (enab==0) printf("Erasing %lx\n",ep->start+i);
          }
          }
          ep=ep->next;
...
case 11:
case 12:
case 14: if (int13(0x42,disk->drive,0L,p->start,&s)) printf("Error on Disk %x Sector %lx\n",disk->drive,p->start);
...
if ((dd->partsecs>1) && (dd->mb==0)) printf("WARNING: Invalid MultiBoot MBR\n");
i=0;
